...
<title>权限管理</title>
 <meta name="renderer" content="webkit">
	...
	/* 修改菜单	 */
		function amendItem(jurisId){
			//alert(itemId);	
			$.ajax({
				type: "post",
	            url: "<%=path%>adminPrivilegeAction/amendInterface.action",
	            dataType: "json",
	            data: {jurisId: jurisId, //发送的数据部分                       
	            },
	            success: function(reData){//接受后台发送的数据
	                    //alert(reData.page);
	                   /*  var jsonObj=eval("("+reData+")");
	                	...
	                	$("#myModal3").prepend("<div class='modal-header'>"                		
	                		+"<button type='button' class='close' data-dismiss='modal' aria-hidden='true'>×</button>" 
	   						...
	   						+"<input name='id' type='hidden' value='"+reData.jurisId+"'>"
	   						+"<div class='layui-form-item'><label class='layui-form-label'>菜单名</label><div class='layui-input-block'>"
	                		+"<input type='text' name='menu' lay-verify='title' autocomplete='off' placeholder='"+reData.menu+"' value='"+reData.menu+"'class='layui-input'></div></div>"
	                		+"<div class='layui-form-item'><label class='layui-form-label'>父ID</label><div class='layui-input-block'>"
	                		+"<input type='text' name='parId' lay-verify='title' autocomplete='off' placeholder='"+reData.parId+"' value='"+reData.parId+"' class='layui-input'></div></div>"

	                		+"<div class='layui-form-item'><label class='layui-form-label'>URL地址</label><div class='layui-input-block'>"
	                		+"<input type='text' name='url' lay-verify='title' autocomplete='off' placeholder='"+reData.url+"' value='"+reData.url+"' class='layui-input'></div></div>"
...
<!--           <input class="layui-input" placeholder="开始日" name="start" id="start">
          ...
          <input type="text" name="menu"  placeholder="${menu }" autocomplete="off" class="layui-input">
      ...
      <div class="page">
        <div>    	
        	共${p }页/当前第${page }页
          <a class="num" href="<%=path%>adminPrivilegeAction/selectPrivilege.action?page=1&menu=${menu}">首页</a>
