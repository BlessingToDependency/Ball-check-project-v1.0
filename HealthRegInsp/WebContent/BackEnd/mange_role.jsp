...
function delectRole(id){
  ...
  <body>
    ...
    <div class="x-body">
      ...
      <xblock>
        <button class="layui-btn" onclick="x_admin_show('新增角色','<%=path%>maRoleAction/toAddRole.action')"><i class="layui-icon"></i>添加</button>
      </xblock>
          ...
          <c:forEach items="${roleList}"  var="rolist"  >
			<tr>
			   <td>${rolist.role}</td>
	       	   ...
	       	   <td class="td-status">
	       	   <span class="layui-btn layui-btn-normal layui-btn-mini"  onclick="x_admin_show('修改角色名','<%=path%>maRoleAction/tomaRole.action?roleId=${rolist.roleId}')">修改</span>
